...
export function CreateGroupForm({organizationId,userId}:{organizationId:string,userId:string}){
 ...
 return <form className="card form" onSubmit={submit}><h2>Novo grupo</h2><div className="field"><label>Nome</label><input className="input" value={name} onChange={e=>setName(e.target.value)} required/></div><div className="field"><label>Descrição</label><textarea className="textarea" value={description} onChange={e=>setDescription(e.target.value)}/></div><button className="btn btn-primary btn-block">Criar grupo</button>{msg&&<div className="error">{msg}</div>}</form>
}
