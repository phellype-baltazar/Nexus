...
export function ProjectScheduleActionsV3({projectId}:{projectId:string}){
 ...
 const gantt=useMemo(()=>{
  ...
  if(zoom==="month"){
   for(let d=new Date(min);d<=max;d=addDays(d,7)){
    const next=addDays(d,7);
    ticks.push({at:new Date(d),label:d.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}),major:`${monthLabel(d)} ${d.getFullYear()}`,width:Math.max(1,(Math.min(next.getTime(),max.getTime()+DAY)-d.getTime())/DAY*px)})
   }
  }else if(zoom==="quarter"){
   for(let d=startMonth(min);d<=max;d=new Date(d.getFullYear(),d.getMonth()+1,1)){
    const next=new Date(d.getFullYear(),d.getMonth()+1,1);
    ticks.push({at:new Date(d),label:monthLabel(d),major:`${q(d)} · ${d.getFullYear()}`,width:Math.max(1,(Math.min(next.getTime(),max.getTime()+DAY)-d.getTime())/DAY*px)})
   }
  }else{
   const firstQuarterMonth=Math.floor(min.getMonth()/3)*3;
   for(let d=new Date(min.getFullYear(),firstQuarterMonth,1);d<=max;d=new Date(d.getFullYear(),d.getMonth()+3,1)){
    const next=new Date(d.getFullYear(),d.getMonth()+3,1);
    const segStart=new Date(Math.max(d.getTime(),min.getTime()));
    const segEnd=Math.min(next.getTime(),max.getTime()+DAY);
    if(segEnd>segStart.getTime())ticks.push({at:segStart,label:q(d),major:String(d.getFullYear()),width:Math.max(1,(segEnd-segStart.getTime())/DAY*px)})
   }
  }
