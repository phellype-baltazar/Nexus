...
export function CheckpointListEditor({reports}:{reports:any[]}){
  ...
  return <>
    <section className="card list">{!reports.length?<div className="empty">Nenhum checkpoint.</div>:reports.map((r:any)=><button type="button" className="row" key={r.id} onClick={()=>setEdit({...r})} style={{width:"100%",border:0,background:"transparent",textAlign:"left",cursor:"pointer"}}><div className="row-main"><div className="row-title">{new Date(`${String(r.period_end).slice(0,10)}T12:00:00`).toLocaleDateString("pt-BR")}</div><div className="row-sub">{r.accomplishments||"Sem resumo"}{r.next_steps?` · próximos passos: ${r.next_steps}`:""}</div>{r.progress!=null&&<div className="muted" style={{fontSize:12,marginTop:5}}>Progresso registrado: {Math.round(Number(r.progress))}%</div>}</div><span className={`chip ${statusClass(r.overall_status)}`}>{statusLabel(r.overall_status)}</span></button>)}</section>
...
export function FinanceGridEditor({organizationId,projectId,budget,items,currency="BRL"}:{organizationId:string;projectId:string;budget:any;items:any[];currency?:string}){
  ...
  return <>
    <section className="grid grid-2">{standards.map(s=><button type="button" className="card" key={s.key} onClick={()=>setEdit({kind:"standard",...s,amount:Number(budget?.[s.key]||0)})} style={{marginTop:0,minWidth:0,display:"flex",flexDirection:"column",justifyContent:"center",textAlign:"center",overflow:"hidden",border:"1px solid var(--line)",cursor:"pointer"}}><div className="eyebrow">{s.label}</div><div style={{fontWeight:900,fontSize:"clamp(16px,4.6vw,22px)",lineHeight:1.05,marginTop:8}}>{fmt(budget?.[s.key])}</div></button>)}
      {items.map((item:any)=><button type="button" className="card" key={item.id} onClick={()=>setEdit({kind:"custom",...item})} style={{marginTop:0,minWidth:0,display:"flex",flexDirection:"column",justifyContent:"center",textAlign:"center",overflow:"hidden",border:"1px solid var(--line)",cursor:"pointer"}}><div className="eyebrow" style={{overflowWrap:"anywhere"}}>{item.label}</div><div style={{fontWeight:900,fontSize:"clamp(16px,4.6vw,22px)",lineHeight:1.05,marginTop:8}}>{fmt(item.amount)}</div>{item.notes&&<div className="muted" style={{fontSize:11,marginTop:7,overflowWrap:"anywhere"}}>{item.notes}</div>}</button>)}
    </section>
