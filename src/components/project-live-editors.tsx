...
function DeleteButton({onDelete,label="Excluir"}:{onDelete:()=>Promise<void>;label?:string}){
...
export function RiskListEditor({risks}:{risks:any[]}){
  ...
  async function save(e:React.FormEvent){e.preventDefault();if(!edit)return;setMsg("");const s=createClient();const isClosed=String(edit.status).startsWith("closed");const {error}=await s.from("risks").update({title:edit.title.trim(),category:edit.category||null,probability:edit.probability,impact:edit.impact,status:edit.status,review_date:edit.review_date||null,mitigation:edit.mitigation||null,contingency:edit.contingency||null,closed_at:isClosed?(edit.closed_at||new Date().toISOString()):null,updated_at:new Date().toISOString()}).eq("id",edit.id);if(error)setMsg(error.message);else location.reload();}
  async function remove(){if(!edit)return;const s=createClient();const {error}=await s.from("risks").update({deleted_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",edit.id);if(error){setMsg(error.message);return;}location.reload();}
  return <>
    <section className="card list">{!risks.length?<div className="empty">Nenhum risco.</div>:risks.map((r:any)=><button type="button" className="row editable-item" key={r.id} onClick={()=>setEdit({...r})} style={{width:"100%",border:0,background:"transparent",textAlign:"left",cursor:"pointer"}}><div className="row-main"><div className="row-title">{r.title}</div><div className="row-sub">{r.category||"Sem categoria"} · {r.probability}/{r.impact} · revisão {r.review_date?new Date(`${r.review_date}T12:00:00`).toLocaleDateString("pt-BR"):"—"}</div><div style={{marginTop:7}}><span className={`chip ${riskStatusClass(String(r.status||"open"))}`}>{riskStatusLabel(String(r.status||"open"))}</span></div></div><span className={`chip ${Number(r.score)>=15?"danger":Number(r.score)>=8?"warning":"success"}`}>{r.score??"—"}</span></button>)}</section>
    {edit&&<Sheet title="Atualizar risco" onClose={()=>setEdit(null)}><form className="card form" style={{margin:0}} onSubmit={save}>
      ...
      <button className="btn btn-primary btn-block">Salvar risco</button><DeleteButton onDelete={remove}/>{msg&&<div className="error">{msg}</div>}
    </form></Sheet>}
  </>;
}
...
export function CheckpointListEditor({reports}:{reports:any[]}){
  ...
  async function remove(){if(!edit)return;const s=createClient();const {error}=await s.from("status_reports").update({deleted_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",edit.id);if(error){setMsg(error.message);return;}location.reload();}
  return <>
    <section className="card list">{!reports.length?<div className="empty">Nenhum checkpoint.</div>:reports.map((r:any)=><button type="button" className="row editable-item" key={r.id} onClick={()=>setEdit({...r})} style={{width:"100%",border:0,background:"transparent",textAlign:"left",cursor:"pointer"}}><div className="row-main"><div className="row-title">{new Date(`${String(r.period_end).slice(0,10)}T12:00:00`).toLocaleDateString("pt-BR")}</div><div className="row-sub">{r.accomplishments||"Sem resumo"}{r.next_steps?` · próximos passos: ${r.next_steps}`:""}</div>{r.progress!=null&&<div className="muted" style={{fontSize:12,marginTop:5}}>Progresso registrado: {Math.round(Number(r.progress))}%</div>}</div><span className={`chip ${statusClass(r.overall_status)}`}>{statusLabel(r.overall_status)}</span></button>)}</section>
    {edit&&<Sheet title="Atualizar checkpoint" onClose={()=>setEdit(null)}><form className="card form" style={{margin:0}} onSubmit={save}>
      ...
      <button className="btn btn-primary btn-block">Salvar checkpoint</button><DeleteButton onDelete={remove}/>{msg&&<div className="error">{msg}</div>}
    </form></Sheet>}
  </>;
}
...
export function FinanceGridEditor({organizationId,projectId,budget,items,currency="BRL"}:{organizationId:string;projectId:string;budget:any;items:any[];currency?:string}){
  ...
  async function save(e:React.FormEvent){e.preventDefault();if(!edit)return;setMsg("");const s=createClient();if(edit.kind==="standard"){
      const amount=Number(edit.amount||0); if(budget?.id){const patch:any={[edit.key]:amount,updated_at:new Date().toISOString()};if(edit.key==="capex_budget")patch.budget=amount+Number(budget.opex_budget||0);if(edit.key==="opex_budget")patch.budget=Number(budget.capex_budget||0)+amount;if(edit.key==="saving_full_year")patch.saving=amount;const {error}=await s.from("budgets").update(patch).eq("id",budget.id);if(error){setMsg(error.message);return;}}
      else {const payload:any={organization_id:organizationId,project_id:projectId,currency,budget:0,actual:0,committed:0,forecast:0,saving:0,benefit:0,capex:0,opex:0,capex_budget:0,opex_budget:0,saving_full_year:0,saving_in_year:0};payload[edit.key]=amount;if(edit.key==="capex_budget"||edit.key==="opex_budget")payload.budget=amount;if(edit.key==="saving_full_year")payload.saving=amount;const {error}=await s.from("budgets").insert(payload);if(error){setMsg(error.message);return;}}
    } else {const {error}=await s.from("project_financial_items").update({label:edit.label.trim(),amount:Number(edit.amount||0),notes:edit.notes||null,updated_at:new Date().toISOString()}).eq("id",edit.id);if(error){setMsg(error.message);return;}}
    location.reload();}
  async function remove(){if(!edit||edit.kind!=="custom")return;const s=createClient();const {error}=await s.from("project_financial_items").update({deleted_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",edit.id);if(error){setMsg(error.message);return;}location.reload();}
  return <>
    <section className="grid grid-2">{standards.map(s=><button type="button" className="card editable-item" key={s.key} onClick={()=>setEdit({kind:"standard",...s,amount:Number(budget?.[s.key]||0)})} style={{marginTop:0,minWidth:0,display:"flex",flexDirection:"column",justifyContent:"center",textAlign:"center",overflow:"hidden",border:"1px solid var(--line)",cursor:"pointer"}}><div className="eyebrow">{s.label}</div><div style={{fontWeight:900,fontSize:"clamp(16px,4.6vw,22px)",lineHeight:1.05,marginTop:8}}>{fmt(budget?.[s.key])}</div></button>)}
      {items.map((item:any)=><button type="button" className="card editable-item" key={item.id} onClick={()=>setEdit({kind:"custom",...item})} style={{marginTop:0,minWidth:0,display:"flex",flexDirection:"column",justifyContent:"center",textAlign:"center",overflow:"hidden",border:"1px solid var(--line)",cursor:"pointer"}}><div className="eyebrow" style={{overflowWrap:"anywhere"}}>{item.label}</div><div style={{fontWeight:900,fontSize:"clamp(16px,4.6vw,22px)",lineHeight:1.05,marginTop:8}}>{fmt(item.amount)}</div>{item.notes&&<div className="muted" style={{fontSize:11,marginTop:7,overflowWrap:"anywhere"}}>{item.notes}</div>}</button>)}
    </section>
    {edit&&<Sheet title={edit.kind==="standard"?`Atualizar ${edit.label}`:"Atualizar item financeiro"} onClose={()=>setEdit(null)}><form className="card form" style={{margin:0}} onSubmit={save}>
      ...
      <button className="btn btn-primary btn-block">Salvar</button>{edit.kind==="custom"&&<DeleteButton onDelete={remove}/>} {edit.kind==="standard"&&<div className="muted" style={{fontSize:12,textAlign:"center"}}>Os quatro indicadores financeiros padrão são fixos; podem ser zerados, mas não excluídos.</div>}{msg&&<div className="error">{msg}</div>}
    </form></Sheet>}
  </>;
}