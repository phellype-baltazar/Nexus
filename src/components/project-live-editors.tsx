...
export function RiskListEditor({risks}:{risks:any[]}){
  ...
  return <>
    ...
    {edit&&<Sheet title="Atualizar risco" onClose={()=>setEdit(null)}><form className="card form" style={{margin:0}} onSubmit={save}>
      ...
      <div className="field"><label>Status</label><select className="select" value={edit.status||"open"} onChange={e=>setEdit({...edit,status:e.target.value})}><option value="open">Open</option><option value="monitoring">Monitoring</option><option value="materialized">Materialized (ocorreu)</option><option value="closed_not_occurred">Closed · did not occur</option><option value="closed_response_complete">Closed · response complete</option></select><div className="muted" style={{fontSize:12,marginTop:6}}>Quando o evento ocorre, deixa de ser incerteza e passa a ser tratado como ocorrido/materializado. Se a janela do risco passou sem ocorrência, feche como “did not occur”.</div></div>
...
export function FinanceGridEditor({budget,items,currency="BRL"}:{budget:any;items:any[];currency?:string}){
  const [edit,setEdit]=useState<any|null>(null); const [msg,setMsg]=useState("");
  ...
  async function save(e:React.FormEvent){e.preventDefault();if(!edit)return;setMsg("");const s=createClient();if(edit.kind==="standard"){if(!budget?.id){setMsg("Crie o registro financeiro padrão primeiro.");return;}const patch:any={[edit.key]:Number(edit.amount||0),updated_at:new Date().toISOString()};if(edit.key==="capex_budget")patch.budget=Number(edit.amount||0)+Number(budget.opex_budget||0);if(edit.key==="opex_budget")patch.budget=Number(budget.capex_budget||0)+Number(edit.amount||0);if(edit.key==="saving_full_year")patch.saving=Number(edit.amount||0);const {error}=await s.from("budgets").update(patch).eq("id",budget.id);if(error){setMsg(error.message);return;}}else{const {error}=await s.from("project_financial_items").update({label:edit.label.trim(),amount:Number(edit.amount||0),notes:edit.notes||null,updated_at:new Date().toISOString()}).eq("id",edit.id);if(error){setMsg(error.message);return;}}location.reload();}
  return <>
