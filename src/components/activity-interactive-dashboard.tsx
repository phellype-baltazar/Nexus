...
export function ActivityInteractiveDashboard(p:Props){
 ...
 return <>
  <section style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:12,marginTop:14,width:"100%"}}>
   <button className="card editable-item" onClick={()=>{setProgressValue(String(Math.round(progress||0)));setEditor("progress")}} style={{...card(),border:"1px solid var(--line)",background:"white"}}><div className="eyebrow">Progresso</div><strong style={{fontSize:size(pText)}}>{pText}</strong></button>
   <button className="card editable-item" onClick={()=>setEditor("status")} style={{...card(),border:`1px solid ${visual.border}`,background:visual.bg,color:visual.color}}><div className="eyebrow" style={{color:visual.color}}>Status</div><strong style={{fontSize:size(visual.label),lineHeight:1.08}}>{visual.label}</strong></button>
   ...
   <button className="card editable-item" onClick={()=>setEditor("owner")} style={{...card(),border:"1px solid var(--line)",background:"white"}}><div className="eyebrow">Responsável</div><strong style={{fontSize:size(ownerName),lineHeight:1.08,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{ownerName}</strong></button>
  ...
  <section className="card" style={{marginTop:12}}>
   <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"start"}}><div><h2 style={{marginBottom:4}}>Workload da atividade</h2><p className="muted" style={{marginTop:0,fontSize:13}}>Horas e prazo originais ficam congelados. Revisões afetam somente o futuro.</p></div>{!terminal&&<button className="chip editable-item" onClick={()=>{setForecastValue(forecastEnd||"");setEditor("forecast")}}>Previsão {dateBR(forecastEnd)}</button>}</div>
   <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:8,marginTop:12}}>
    ...
    <button className="editable-item" disabled={terminal} onClick={()=>{setRemainingValue(String(Number(remainingWorkHours??futurePlanned)));setEditor("remaining")}} style={{padding:10,border:"1px solid var(--line)",borderRadius:14,textAlign:"center",background:"white"}}><div className="eyebrow">Carga futura</div><strong>{fmtHours(futurePlanned)}</strong><div className="muted" style={{fontSize:9}}>toque para revisar</div></button>
    <button className="editable-item" disabled={terminal} onClick={()=>{setForecastValue(forecastEnd||"");setEditor("forecast")}} style={{padding:10,border:"1px solid var(--line)",borderRadius:14,textAlign:"center",background:"white"}}><div className="eyebrow">Prazo revisado</div><strong style={{fontSize:13}}>{dateBR(forecastEnd)}</strong><div className="muted" style={{fontSize:9}}>toque para revisar</div></button>
   </div>
  ...
  <section className="card" style={{marginTop:12}}><h2 style={{marginBottom:4}}>Distribuição do esforço</h2><p className="muted" style={{marginTop:0,fontSize:13}}>A curva distribui a carga futura entre hoje e o prazo revisado. Ao ampliar o prazo, a carga diária diminui visualmente.</p>
   <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:8,marginTop:12}}>{PROFILES.map(x=>{const active=effortProfile===x.key;return <button className="editable-item" key={x.key} disabled={busy||terminal} onClick={()=>chooseProfile(x.key)} style={{minHeight:88,borderRadius:16,border:`1px solid ${active?"var(--primary)":"var(--line)"}`,background:active?"#edf4ff":"white",color:active?"var(--primary)":"var(--text)",padding:7,fontWeight:800,opacity:terminal?.6:1}}><Curve path={x.path}/><div style={{fontSize:11}}>{x.short}</div></button>})}</div>
   {effortProfile==="shifted_peak"&&!terminal&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:10}}><button className={`btn editable-item ${timing==="early"?"btn-primary":"btn-outline"}`} onClick={()=>chooseProfile("shifted_peak","early")}>Pico Early</button><button className={`btn editable-item ${timing==="late"?"btn-primary":"btn-outline"}`} onClick={()=>chooseProfile("shifted_peak","late")}>Pico Late</button></div>}
