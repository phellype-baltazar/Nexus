...
export function StructureBuilder({organizationId,userId}:{organizationId:string;userId:string}) {
  const [group,setGroup]=useState(""), [program,setProgram]=useState(""), [project,setProject]=useState("");
  const [groupDesc,setGroupDesc]=useState(""), [programObjective,setProgramObjective]=useState(""), [projectDesc,setProjectDesc]=useState("");
  ...
  async function submit(e:React.FormEvent){
    ...
    try{
      const slug=group.trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g,"-")+"-"+Math.random().toString(36).slice(2,6);
      const {data:g,error:ge}=await s.from("groups").insert({organization_id:organizationId,name:group.trim(),description:groupDesc||null,slug,owner_user_id:userId}).select("id").single();
      if(ge) throw ge;
      const {data:p,error:pe}=await s.from("programs").insert({organization_id:organizationId,group_id:g.id,name:program.trim(),objective:programObjective||null,owner_user_id:userId,start_date:projectStart||null,due_date:projectEnd||null}).select("id").single();
      if(pe) throw pe;
    ...
    }catch(err:any){setMsg(err?.message||"Não foi possível criar a estrutura.");setBusy(false)}
  }

  return <form className="form" onSubmit={submit}>
    <section className="card form"><span className="eyebrow">1 · Grupo</span><div className="field"><label>Nome do grupo/direção</label><input className="input" value={group} onChange={e=>setGroup(e.target.value)} required/></div><div className="field"><label>Descrição</label><textarea className="textarea" value={groupDesc} onChange={e=>setGroupDesc(e.target.value)}/></div></section>
