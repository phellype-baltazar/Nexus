...
export function ProjectScheduleActions({projectId}:{projectId:string}){
 ...
 function changeZoom(z:ZoomMode){setZoom(z);requestAnimationFrame(()=>{if(scrollRef.current)scrollRef.current.scrollLeft=0})}

 const gantt=useMemo(()=>{
  const dates=rows.flatMap(r=>[r.start_date,r.finish_date,r.early_start,r.early_finish]).filter(Boolean) as string[];if(!dates.length)return null;
  const min=startOfMonth(new Date(`${[...dates].sort()[0]}T12:00:00`)),max=endOfMonth(new Date(`${[...dates].sort().at(-1)!}T12:00:00`));
  const months:Date[]=[];for(let c=startOfMonth(min);c<=max;c=addMonths(c,1))months.push(new Date(c));
  const monthWidth=zoom==="month"?180:zoom==="quarter"?82:34;
  const timelineWidth=Math.max(520,months.length*monthWidth),totalDays=Math.max(1,Math.round((max.getTime()-min.getTime())/DAY)+1),dayPx=timelineWidth/totalDays;
  const quarterGroups:Array<{year:number;q:string;span:number}>=[];months.forEach(m=>{const k=`${m.getFullYear()}-${quarter(m)}`,p=quarterGroups[quarterGroups.length-1];if(p&&`${p.year}-${p.q}`===k)p.span++;else quarterGroups.push({year:m.getFullYear(),q:quarter(m),span:1})});
  const yearGroups:Array<{year:number;span:number}>=[];months.forEach(m=>{const p=yearGroups[yearGroups.length-1];if(p&&p.year===m.getFullYear())p.span++;else yearGroups.push({year:m.getFullYear(),span:1})});
  const x=(v?:string|null)=>{if(!v)return 0;return Math.max(0,Math.min(timelineWidth,(new Date(`${v}T12:00:00`).getTime()-min.getTime())/DAY*dayPx))};
  const width=(a?:string|null,b?:string|null)=>!a||!b?6:Math.max(6,x(b)-x(a)+dayPx);
  return{min,max,months,quarterGroups,yearGroups,monthWidth,timelineWidth,x,width};
 },[rows,zoom]);
 ...
 const zoomButton=(z:ZoomMode):React.CSSProperties=>({borderRadius:999,border:zoom===z?"2px solid #2563eb":"1px solid #cbd5e1",background:zoom===z?"#2563eb":"#fff",color:zoom===z?"#fff":"#475569",padding:"9px 14px",fontWeight:900,boxShadow:zoom===z?"0 4px 12px rgba(37,99,235,.2)":"none"});
 ...
 return <>
  ...
  {modal&&<div onClick={()=>setModal(null)} style={{position:"fixed",inset:0,zIndex:120,background:"rgba(15,23,42,.46)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}><div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:1000,maxHeight:"94dvh",overflowY:"auto",background:"#f6f8fb",borderRadius:"26px 26px 0 0",padding:"14px 12px calc(22px + env(safe-area-inset-bottom,0px))"}}>
   ...
   {modal==="gantt"&&gantt&&rows.length>0&&<>
    <div style={{display:"flex",justifyContent:"space-between",gap:8,flexWrap:"wrap",marginBottom:10}}><div style={{display:"flex",gap:6,flexWrap:"wrap"}}><span className="chip" style={{background:"#e2e8f0",color:"#475569"}}>━ Baseline</span><span className="chip" style={{background:"#dbeafe",color:"#1d4ed8"}}>▬ Forecast / progresso</span><span className="chip" style={{background:"#fee2e2",color:"#991b1b"}}>● Crítica</span><span className="chip" style={{background:"#fef3c7",color:"#92400e"}}>● Quase crítica</span></div></div>
    <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10,flexWrap:"wrap"}}><strong style={{fontSize:12,color:"#475569"}}>Escala:</strong>{(["month","quarter","year"] as ZoomMode[]).map(z=><button key={z} type="button" aria-pressed={zoom===z} onClick={()=>changeZoom(z)} style={zoomButton(z)}>{z==="month"?"Mês":z==="quarter"?"Trimestre":"Ano"}{zoom===z?" ✓":""}</button>)}</div>
    <div className="card" style={{padding:0,overflow:"hidden",marginTop:0}}><div ref={scrollRef} style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}><div style={{width:LEFT+gantt.timelineWidth,minWidth:LEFT+gantt.timelineWidth}}>
      <div style={{display:"grid",gridTemplateColumns:`${LEFT}px ${gantt.timelineWidth}px`,position:"sticky",top:0,zIndex:20,background:"white",borderBottom:"1px solid var(--line)"}}>
       ...
       <div>
        {zoom==="year"?<><div style={{display:"flex",height:34,borderBottom:"1px solid var(--line)"}}>{gantt.yearGroups.map((g,i)=><div key={i} style={{width:g.span*gantt.monthWidth,borderRight:"1px solid var(--line)",display:"grid",placeItems:"center",fontSize:12,fontWeight:900}}>{g.year}</div>)}</div><div style={{display:"flex",height:34}}>{gantt.quarterGroups.map((g,i)=><div key={i} style={{width:g.span*gantt.monthWidth,borderRight:"1px solid var(--line)",display:"grid",placeItems:"center",fontSize:10,fontWeight:800,color:"#64748b"}}>{g.q}</div>)}</div></>:<><div style={{display:"flex",height:34,borderBottom:"1px solid var(--line)"}}>{gantt.quarterGroups.map((g,i)=><div key={i} style={{width:g.span*gantt.monthWidth,borderRight:"1px solid var(--line)",display:"grid",placeItems:"center",fontSize:11,fontWeight:900}}>{g.q} · {g.year}</div>)}</div><div style={{display:"flex",height:34}}>{gantt.months.map((m,i)=><div key={i} style={{width:gantt.monthWidth,borderRight:"1px solid var(--line)",display:"grid",placeItems:"center",fontSize:10,fontWeight:800,color:"#64748b"}}>{monthLabel(m)}{zoom==="month"?` ${m.getFullYear()}`:""}</div>)}</div></>}
       </div>
      </div>
      {rows.map((r,index)=>{const bs=r.start_date,be=r.finish_date,fs=r.early_start||bs,fe=r.early_finish||be,bl=gantt.x(bs),bw=gantt.width(bs,be),fl=gantt.x(fs),fw=gantt.width(fs,fe),pct=Math.max(0,Math.min(100,r.progress||0)),c=color(r),changed=bs!==fs||be!==fe;return <div key={r.activity_id} style={{display:"grid",gridTemplateColumns:`${LEFT}px ${gantt.timelineWidth}px`,minHeight:88,borderBottom:"1px solid var(--line)",background:index%2?"#fbfdff":"white"}}>
       ...
       <div style={{position:"relative",height:88,backgroundImage:`repeating-linear-gradient(to right,transparent 0,transparent ${Math.max(1,gantt.monthWidth-1)}px,rgba(148,163,184,.2) ${gantt.monthWidth}px)`}}>
