...
async function switchWorkspace(formData: FormData) {
  ...
  const { data, error } = await s.rpc("rpc_set_current_workspace", {
    p_organization_id: organizationId,
    p_group_id: null,
  });

  if (error || data !== true) {
    redirect("/app/workspace?switch=error");
  }
...
export default async function Page({
  ...
  if (
    ...
    const { data } = await s.rpc("rpc_get_workspace_invite_code", {
      p_organization_id: w.id,
    });
  ...
  const workspaces = (myMemberships || [])
    .map((membership: any) => ({
    ...
    <main className="page">
      ...
      <section className="card">
        <Link
          ...
          href="/onboarding"
        >
          Criar ou entrar em outro workspace
        </Link>
      </section>
    </main>
  );
