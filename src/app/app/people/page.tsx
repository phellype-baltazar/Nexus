    ...
    <section className="card list">
      {!req?.length?<div className="empty">Nenhuma solicitação pendente.</div>:req.map((r:any)=><div className="row" key={r.request_id} style={{alignItems:"flex-start"}}>
        <div className="row-main">
          <div className="row-title">{r.requester_name||r.requester_email||"Usuário"}</div>
        ...
        {canManage&&<AccessDecisionButtons requestId={r.request_id} requestedRole={r.requested_role||"member"}/>} 
      </div>)}
    </section>
